...
export default function RunDetailPage({ params }: PageProps) {
  ...
  const { run: polledRun } = usePollRun(shouldPoll ? runId : null, 5_000);
  ...
  const hasPlaywrightArtifact = artifacts.some((a) =>
    /playwright|test.?results?|pw.?results?/i.test(a.name),
  );
        ...
        {run && (
          <>
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-zinc-300">Run #{run.run_number}</span>
          ...
          {run.status === 'completed' && (
            <Link
              ...
              className={`flex items-center justify-between p-4 rounded-xl border transition-all group ${
                hasPlaywrightArtifact
                  ? 'bg-indigo-600/10 border-indigo-500/30 hover:bg-indigo-600/15 hover:border-indigo-500/50'
                  : 'bg-zinc-900 border-zinc-800 hover:border-zinc-700'
              }`}
            >
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg ${hasPlaywrightArtifact ? 'bg-indigo-500/20 text-indigo-400' : 'bg-zinc-800 text-zinc-500'}`}>
                  <FlaskConical className="w-4 h-4" />
                </div>
                <div>
                  <p className={`text-sm font-semibold ${hasPlaywrightArtifact ? 'text-indigo-300' : 'text-zinc-400'}`}>
                    View Playwright Test Results
                  </p>
                  <p className="text-xs text-zinc-500 mt-0.5">
                    {hasPlaywrightArtifact
                      ? 'Inline test results, screenshots & failure details'
                      : 'No playwright-results artifact found for this run'}
                  </p>
                </div>
              </div>
